          ...
          return (
            <button key={opt} type="button" onClick={() => onToggle(opt)}
              className={`px-3 py-1 rounded-full text-xs font-medium border transition-all ${
                active ? "bg-[#B8960C] border-[#B8960C] text-white shadow-sm"
...
function TypeChipGroup({ label, options, selected, onToggle }: {
...
export function EditClientForm({ client, onSuccess, onCancel }: EditClientFormProps) {
  ...
  const [form, setForm] = useState({
    ...
    priceGroup:  (client.priceGroup ?? "") as PriceGroup | "",
    budgetMin:   client.budgetMin != null ? String(client.budgetMin) : "",
    budgetMax:   client.budgetMax != null ? String(client.budgetMax) : "",
    stage:       client.stage,
    primaryAgent: client.primaryAgent ?? "",
    propertyNotes: "",
  });

  const [locations,     setLocations]     = useState<string[]>(client.propertyLocations ?? []);
  const [propertyTypes, setPropertyTypes] = useState<string[]>(client.propertyTypes ?? []);
  const [bedroomsMin,   setBedroomsMin]   = useState(client.propertyBedroomsMin ?? "");
  const [bedroomsMax,   setBedroomsMax]   = useState(client.propertyBedroomsMax ?? "");
  const [pool,          setPool]          = useState<"any" | "yes" | "no">(client.propertyPool ?? "any");
  const [views,         setViews]         = useState<string[]>(client.propertyViews ?? []);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const updated: Client = {
      ...
      clientClass:  form.clientClass as ClientClass,
  ...
  return (
    <form onSubmit={handleSubmit} noValidate className="px-6 py-5 space-y-6">
      ...
      <FieldSet title="Budget">
        <div className="grid grid-cols-2 gap-4">
          <Input label="Budget Min (€)" type="number" value={form.budgetMin}
            onChange={e => set("budgetMin", e.target.value)} hint="Minimum in euros" />
          <Input label="Budget Max (€)" type="number" value={form.budgetMax}
      ...
      <FieldSet title="Property Interest">
        ...
        />
        <div className="grid grid-cols-2 gap-4">
          <Select label="Bedrooms Min" value={bedroomsMin}
            onChange={e => setBedroomsMin(e.target.value)} options={BEDROOM_OPTIONS} />
          <Select label="Bedrooms Max" value={bedroomsMax}
            onChange={e => setBedroomsMax(e.target.value)} options={BEDROOM_OPTIONS} />
        </div>
        <div className="flex flex-col gap-1.5">
          ...
          <div className="flex gap-2">
            {(["any", "yes", "no"] as const).map(opt => (
              <button key={opt} type="button" onClick={() => setPool(opt)}
                className={`px-4 py-1.5 rounded-full text-xs font-medium border transition-all ${
                  pool === opt ? "bg-[#B8960C] border-[#B8960C] text-white shadow-sm"
                               : "bg-white border-warm-300 text-warm-700 hover:border-[#B8960C] hover:text-[#B8960C]"
                }`}>
        ...
        />
        <div className="flex flex-col gap-1.5">
          <label className="text-[11px] font-semibold uppercase tracking-widest text-warm-700">Notes</label>
          <textarea
            rows={2}
